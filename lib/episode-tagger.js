...
[{"tags": ["...", "..."]}, ...]`;
  ...
  try {
    ...
    console.error('[episode-tagger] fetch failed:', String(err.message || err));
    return null;
  }
  if (!res.ok) {
    console.error('[episode-tagger] Perplexity non-OK response:', res.status, (await res.text()).slice(0, 500));
    return null;
  }
  // Everything past this point is defensive: any unexpected shape in the AI
  // response must return null + a log line, never an uncaught rejection (a
  // rejected chunk promise silently drops its reason in Promise.allSettled).
  try {
    ...
    if (!Array.isArray(arr)) {
      console.error('[episode-tagger] could not parse model output as JSON array:', raw.slice(0, 500));
      return null;
    }
    if (arr.length !== episodes.length) {
      console.error(`[episode-tagger] length mismatch: expected ${episodes.length}, got ${arr.length}`);
      return null;
    }
    ...
    console.error('[episode-tagger] unexpected error parsing response:', String(err.message || err));
    return null;
  }
}
...
export async function tagUntaggedEpisodes({ claimSize = 300, trigger = 'backfill' } = {}) {
  ...
  for (let ci = 0; ci < chunks.length; ci++) {
    ...
    if (res.status === 'rejected') {
      console.error('[episode-tagger] chunk promise rejected:', String(res.reason?.message || res.reason));
      continue;
    }
    if (!res.value) continue; // tagChunk already logged the specific reason

    // Plain per-row UPDATEs, not upsert() — episodes.id is a GENERATED ALWAYS
    // IDENTITY column, so any upsert() (which issues INSERT ... ON CONFLICT
    // under the hood) is rejected by Postgres outright, even though every one
    // of these rows already exists. Run concurrently since these are cheap
    // single-row updates by primary key.
    const rows = chunk.map((ep, idx) => ({ id: ep.id, quiz_tags: res.value[idx] || [], quiz_tagged_at: nowIso }));
    const updateResults = await Promise.all(rows.map(r =>
      db.from('episodes').update({ quiz_tags: r.quiz_tags, quiz_tagged_at: r.quiz_tagged_at }).eq('id', r.id)
    ));
    updateResults.forEach((r, idx) => {
      if (r.error) {
        console.error('[episode-tagger] update failed for episode', rows[idx].id, ':', r.error.message);
        return;
      }
      tagged += 1;
      sample.push({ title: chunk[idx].title, show: chunk[idx].show_name, tags: rows[idx].quiz_tags });
    });
  }
