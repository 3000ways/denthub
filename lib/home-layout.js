...
// The Discover block is a *container*: it renders several episode carousels, one
// per quiz-option tag, grouped into three kinds. These are the default number of
// rows shown per kind (the composer lets an admin change them per audience and
// hide individual tags). Single source of truth, shared by the composer and the
// feed builder.
export const DISCOVER_DEFAULT_COUNTS = { goal: 5, interest: 2, career: 1 };
export const DISCOVER_KINDS = [
  { kind: 'goal',     label: 'Listen by goal',   quizKey: 'working_on' },
  { kind: 'interest', label: 'By clinical area', quizKey: 'interest' },
  { kind: 'career',   label: 'By career stage',  quizKey: 'career_stage' },
];

// "Your carousels" (the `personal` block) is also a container, but its rows are
// built from each signed-in dentist's OWN profile answers — not a global topic
// list — so the admin can only set how many rows of each kind to show (0 hides a
// kind for everyone). `eyebrow` is the label visitors see above each row.
export const PERSONAL_DEFAULT_COUNTS = { goal: 3, interest: 2, career: 1 };
export const PERSONAL_KINDS = [
  { kind: 'goal',     label: 'What they’re working on', eyebrow: 'Because you’re working on' },
  { kind: 'interest', label: 'Their clinical interests', eyebrow: 'More on' },
  { kind: 'career',   label: 'Their career stage',       eyebrow: 'For your stage' },
];
