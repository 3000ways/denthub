...
export async function harvestBatch({ limit = 75, timeBudgetMs = 45000 } = {}) {
  ...
  for (const { show_resource_id } of queue) {
    ...
    try {
      // Cap any single show's RSS pagination so it can't overrun the function:
      // whichever comes first — the overall budget, or ~12s on this one feed.
      const deadline = Math.min(startedAt + timeBudgetMs, Date.now() + 12000);
      const episodes = await fetchAllEpisodes(show.rssUrl, deadline);
