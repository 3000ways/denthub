...
async function updateAirtableAutoImage(showId, showArt) {
  if (!showArt || !AIRTABLE_PAT) return;
  try {
    await fetch(`https://api.airtable.com/v0/${AIRTABLE_BASE}/${AIRTABLE_TABLE}/${showId}`, {
      method: 'PATCH',
      headers: { Authorization: `Bearer ${AIRTABLE_PAT}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ fields: { 'Auto Image URL': showArt } }),
...
export async function harvestShow(showId, { throttleMs = 6 * 60 * 60 * 1000 } = {}) {
  ...
  if (!feedUrlRaw) {
    if (!AIRTABLE_PAT) return { skipped: true, reason: 'no_feed' };
    try {
      const r = await fetch(`https://api.airtable.com/v0/${AIRTABLE_BASE}/${AIRTABLE_TABLE}/${showId}`, {
        headers: { Authorization: `Bearer ${AIRTABLE_PAT}` },
        signal: AbortSignal.timeout(8000),
      });
      if (r.ok) {
...
export async function repairMissingFeeds({ limit = 5, timeBudgetMs = 15000 } = {}) {
  if (!AIRTABLE_PAT) return { repaired: 0, checked: 0, skipped: 'no_pat' };
  const startedAt = Date.now();

  let records = [];
  try {
    const params = new URLSearchParams();
    params.set('filterByFormula', `AND({Type}="Podcast", {Status}="Published", {RSS Feed URL}="")`);
    params.set('pageSize', '100');
    ...
    if (res.ok) { const j = await res.json(); records = j.records || []; }
  } catch { return { repaired: 0, checked: 0, error: 'airtable_read' }; }
  ...
  for (const rec of records) {
    ...
    try {
      await fetch(`https://api.airtable.com/v0/${AIRTABLE_BASE}/${AIRTABLE_TABLE}/${rec.id}`, {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${AIRTABLE_PAT}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ fields: { 'RSS Feed URL': hit.feedUrl } }),
        signal: AbortSignal.timeout(8000),
      });
      repaired += 1;
