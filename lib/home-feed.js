...
// How many rows of each kind to feature per day (goal-heavy on purpose).
const COUNTS = { goal: 5, interest: 2, career: 1 };
...
export async function buildPersonalFeed(profile) {
  if (!profile) return [];
  const specs = [
    ...(profile.focus_areas || []).slice(0, 3).map(tag => ({ kind: 'goal', tag, eyebrow: 'Because you’re working on' })),
    ...(profile.interests || []).slice(0, 2).map(tag => ({ kind: 'interest', tag, eyebrow: 'More on' })),
    ...(profile.career_stage ? [{ kind: 'career', tag: profile.career_stage, eyebrow: 'For your stage' }] : []),
  ];
  return (await Promise.all(specs.map(fetchRow))).filter(Boolean);
}

// The full rotating discovery feed for the logged-out home page.
export async function buildDiscoverFeed(daySeed = 0) {
  const opts = await fetchQuizOptions();
  const specs = [
    ...rotatePick(opts[QUESTION_KEYS.WORKING_ON] || [], COUNTS.goal, daySeed).map(tag => ({ kind: 'goal', tag })),
    ...rotatePick(opts[QUESTION_KEYS.INTEREST] || [], COUNTS.interest, daySeed).map(tag => ({ kind: 'interest', tag })),
    ...rotatePick(opts[QUESTION_KEYS.CAREER_STAGE] || [], COUNTS.career, daySeed).map(tag => ({ kind: 'career', tag })),
  ];
