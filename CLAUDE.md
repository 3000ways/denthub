...
- The old separate **Featured / Recent / Notable** sections were collapsed into a single **"All Episodes"** list (`components/AllEpisodes.js`, DB-backed, newest-first, searchable). The creator's **featured episodes** (owner picks in `resource_owner_content.featured_episode_ids`) are **pinned + `★ Featured`-badged at the top** of that one list; there is no standalone Featured section. (No AI-featured system exists yet — featured = owner picks only. A future carousel can query these.)
