...
- **Data model — two Airtable fields on Resources (editorial record):** `Voice Type` (`fldZITWmPkoM7KpaG` — Human / AI-generated / Mixed) and `Voice Status` (`fldT89FBBmiioyUTb` — Suspected / Confirmed). `Suspected` is a listener/bot guess and stays **internal only**. Andrei sets these in the admin **All Resources** editor (a "🤖 AI Voice disclosure" block); the collapsed card shows a 🤖 chip (amber = confirmed/public, grey = suspected/internal).
