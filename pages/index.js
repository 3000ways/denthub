...
function ScoreBadge({ score, fields }) {
  ...
  return (
    ...
    >
      <span style={{ display:'inline-block', fontSize:11, fontWeight:500, color:'#085041', background:'#E1F5EE', padding:'2px 7px', borderRadius:3, cursor: hasBreakdown ? 'default' : 'default', userSelect:'none' }}>
        ★ {score}
      </span>
      {show && hasBreakdown && (
        <div style={{
          position:'absolute', bottom:'calc(100% + 8px)', left:'50%', transform:'translateX(-50%)',
          background:'#fff', border:'0.5px solid #e0e0e0', borderRadius:8, padding:'10px 14px',
          width:210, boxShadow:'0 4px 20px rgba(0,0,0,0.12)', zIndex:999, pointerEvents:'none',
        }}>
          <div style={{ fontSize:11, fontWeight:600, color:'#111', marginBottom:8, paddingBottom:6, borderBottom:'0.5px solid #f0f0f0' }}>
            Score breakdown
          </div>
          {breakdown.map(b => (
            <div key={b.label} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:5 }}>
              ...
              <span style={{ fontSize:11, fontWeight:500, color: b.value != null ? '#0F6E56' : '#bbb' }}>
                {b.value ?? '—'}
              </span>
            </div>
          ))}
          ...
          <div style={{
            ...
            width:8, height:8, background:'#fff', border:'0.5px solid #e0e0e0',
  ...
  return (
    ...
    <div style={s.inner}>
      ...
      <div style={s.themeNav}>
        <button onClick={() => selectTheme(null)} style={{
          fontSize:13, padding:'0 0 11px', marginRight:24, background:'none', border:'none',
          borderBottom: !activeTheme ? '2px solid #0F6E56' : '2px solid transparent',
          color: !activeTheme ? '#111' : '#888', fontWeight: !activeTheme ? 500 : 400,
          cursor:'pointer', fontFamily:'inherit', whiteSpace:'nowrap'
        }}>All resources</button>
        {THEMES.map(t => (
          <button key={t} onClick={() => selectTheme(activeTheme === t ? null : t)} style={{
            fontSize:13, padding:'0 0 11px', marginRight:24, background:'none', border:'none',
            borderBottom: activeTheme === t ? '2px solid #0F6E56' : '2px solid transparent',
            color: activeTheme === t ? '#111' : '#888', fontWeight: activeTheme === t ? 500 : 400,
            cursor:'pointer', fontFamily:'inherit', whiteSpace:'nowrap'
          }}>{THEME_SHORT[t]}</button>
        ))}
      </div>
                  ...
                  <div key={r.id} style={{ borderLeft: i===1 ? '1px solid #e8e8e8' : 'none', paddingLeft: i===1 ? 32 : 0 }}>
                    <div onClick={() => r.fields.URL && window.open(r.fields.URL, '_blank')} style={{ cursor:'pointer' }}>
                      ...
                      <div style={{ fontSize:13, color:'#666', lineHeight:1.55 }}>
                        {(r.fields.Description||'').slice(0,140)}{(r.fields.Description||'').length > 140 ? '…' : ''}
                      </div>
