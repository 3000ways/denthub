  ...
  if (imageUrl) {
    return (
      <img src={imageUrl} alt={name} onError={() => setErr(true)}
        style={{ width:sz*0.72, height:sz, borderRadius:3, objectFit:'cover', flexShrink:0, boxShadow:'2px 2px 6px rgba(0,0,0,0.15)' }} />
    );
  }
...
function ScoreBadge({ score, fields }) {
  ...
  return (
    <span style={{ position:'relative', display:'inline-block' }} onMouseEnter={() => setShow(true)} onMouseLeave={() => setShow(false)}>
      <span style={{ display:'inline-block', fontSize:11, fontWeight:500, color:'#085041', background:'#E1F5EE', padding:'2px 7px', borderRadius:3, userSelect:'none' }}>★ {score}</span>
      {show && hasData && (
...
export default function Home() {
  ...
  return (
    <div style={s.page}><div style={s.inner}>
      ...
      <div style={s.topbar}>
        <div style={{ fontSize:15, fontWeight:500, color:'#111', letterSpacing:-0.3 }}>Dent<span style={{ color:'#0F6E56' }}>Hub</span></div>
        <div style={{ display:'flex', gap:20 }}>
          {['Learning','Technology','Coaching','Specialty'].map(t => {
            ...
            return <a key={t} href="#" onClick={e => { e.preventDefault(); selectTheme(isActive ? null : full); }} style={{ fontSize:13, color: isActive ? '#111':'#888', textDecoration:'none', fontWeight: isActive ? 500:400 }}>{t}</a>;
          })}
        </div>
      ...
      {activeTheme && (
        <div style={{ paddingTop:8, marginBottom:24 }}>
          ...
          <h1 style={{ ...s.h1, fontSize:24 }}>{activeCategory ? activeCategory : `${themeCats.length} categories · ${filtered.length} resources`}</h1>
        </div>
      )}
      ...
      <div style={s.themeNav}>
        <button onClick={() => selectTheme(null)} style={{ fontSize:13, padding:'0 0 11px', marginRight:24, background:'none', border:'none', borderBottom: !activeTheme ? '2px solid #0F6E56':'2px solid transparent', color: !activeTheme ? '#111':'#888', fontWeight: !activeTheme ? 500:400, cursor:'pointer', fontFamily:'inherit', whiteSpace:'nowrap' }}>All resources</button>
        {THEMES.map(t => (
          <button key={t} onClick={() => selectTheme(activeTheme===t ? null:t)} style={{ fontSize:13, padding:'0 0 11px', marginRight:24, background:'none', border:'none', borderBottom: activeTheme===t ? '2px solid #0F6E56':'2px solid transparent', color: activeTheme===t ? '#111':'#888', fontWeight: activeTheme===t ? 500:400, cursor:'pointer', fontFamily:'inherit', whiteSpace:'nowrap' }}>{THEME_SHORT[t]}</button>
        ))}
      </div>
      ...
      {!loading && (<>
        ...
        {top2.length > 0 && (<>
          <div style={s.sectionLbl}>{activeCategory ? `Top ${activeCategory}` : activeTheme ? `Top ${THEME_SHORT[activeTheme]} picks` : 'Top picks this week'}</div>
            ...
            {top2.map((r, i) => (
              <div key={r.id} style={{ borderLeft: i===1 ? '1px solid #e8e8e8':'none', paddingLeft: i===1 ? 32:0 }}>
                <div onClick={() => r.fields.URL && window.open(r.fields.URL,'_blank')} style={{ cursor:'pointer' }}>
                  ...
                  <div style={{ marginTop:10 }}><ScoreBadge score={(r.fields['Final Score']||0).toFixed(1)} fields={r.fields} /></div>
                </div>
              </div>
            ))}
            {top2.length===1 && <div />}
        ...
        {ranked.length > 0 && (<>
          <div style={s.sectionLbl}>{activeCategory ? `${activeCategory} — ranked` : 'Top 50 ranked'}</div>
        ...
        {ranked.length===0 && <div style={{ padding:'40px 0', textAlign:'center', color:'#aaa', fontSize:14 }}>No resources yet in this category.</div>}
