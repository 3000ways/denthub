...
export default function Home() {
    ...
    async function fetchData() {
      try {
        const [resRes, catRes] = await Promise.all([
          fetch(`https://api.airtable.com/v0/${BASE_ID}/Resources?filterByFormula={Status}='Published'&sort[0][field]=Final Score&sort[0][direction]=desc`, {
            headers: { Authorization: `Bearer ${AIRTABLE_PAT}` }
          }),
          fetch(`https://api.airtable.com/v0/${BASE_ID}/Categories?filterByFormula={Visible}=1&sort[0][field]=Display Order`, {
            headers: { Authorization: `Bearer ${AIRTABLE_PAT}` }
          }),
        ]);
  ...
  const featured   = filteredResources.slice(0, 1)[0];
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '0 20px 60px' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'16px 0 8px' }}>
          ...
          <div style={{ display:'flex', gap:8 }}>
            <button style={{ fontSize:12, fontWeight:500, padding:'5px 14px', borderRadius:20, border:'1px solid #0F6E56', background:'#0F6E56', color:'#fff', cursor:'pointer' }}>
              Submit a resource
            </button>
            <button style={{ fontSize:12, fontWeight:500, padding:'5px 14px', borderRadius:20, border:'1px solid #ddd', background:'#fff', color:'#333', cursor:'pointer' }}>
              Sign in
            </button>
          </div>
        </div>
        ...
        <div style={{ padding:'24px 0 16px' }}>
          ...
          <h1 style={{ fontSize:28, fontWeight:500, color:'#111', lineHeight:1.25, marginBottom:8, margin:'0 0 8px' }}>
        ...
        <div style={{ marginBottom:8, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <span style={{ fontSize:12, fontWeight:500, textTransform:'uppercase', letterSpacing:'0.08em', color:'#999' }}>Browse by category</span>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(120px, 1fr))', gap:10, marginBottom:28 }}>
          {displayCats.slice(0,6).map(name => {
            ...
            return (
              <div key={name} onClick={() => setActiveTab(name)} style={{
              ...
              >
                <div style={{ height:70, background:cfg.bg, display:'flex', alignItems:'center', justifyContent:'center', fontSize:28 }}>
                  {cfg.emoji}
                </div>
        ...
        {loading && <div style={{ textAlign:'center', padding:40, color:'#999' }}>Loading resources…</div>}
        ...
        {!loading && (
          <>
            ...
            {featured && (
              <>
                <div style={{ marginBottom:8, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                  <span style={{ fontSize:12, fontWeight:500, textTransform:'uppercase', letterSpacing:'0.08em', color:'#999' }}>Editor's picks</span>
                </div>
                <div style={{ display:'grid', gridTemplateColumns:'1.6fr 1fr', gap:14, marginBottom:28 }}>
                  <div style={{ background:'#fff', border:'1px solid #e8e8e8', borderRadius:12, overflow:'hidden', cursor:'pointer' }}
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                    {sideItems.map(r => {
                      ...
                      return (
                        <div key={r.id} onClick={() => r.fields.URL && window.open(r.fields.URL, '_blank')} style={{
                          background:'#fff', border:'1px solid #e8e8e8', borderRadius:12, overflow:'hidden', display:'flex', cursor:'pointer', minHeight:72
                        }}>
            ...
            {/* Top 10 ranked list */}
            <div style={{ marginBottom:8, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              <span style={{ fontSize:12, fontWeight:500, textTransform:'uppercase', letterSpacing:'0.08em', color:'#999' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              {rankItems.map((r, i) => {
                ...
                return (
                  <div key={r.id} onClick={() => r.fields.URL && window.open(r.fields.URL, '_blank')} style={{
                    display:'flex', alignItems:'center', gap:10, background:'#fff', border:'1px solid #e8e8e8',
                    borderRadius:12, padding:'9px 12px', cursor:'pointer',
                  }}
                  onMouseEnter={e => e.currentTarget.style.borderColor='#1D9E75'}
                  onMouseLeave={e => e.currentTarget.style.borderColor='#e8e8e8'}
                  >
                    <div style={{ fontSize:18, fontWeight:500, minWidth:22, textAlign:'center', color:rankColor, lineHeight:1 }}>{i+1}</div>
